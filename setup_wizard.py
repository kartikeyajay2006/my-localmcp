#!/usr/bin/env python3
"""Guided terminal installer for my-localmcp.
...
(``my_localmcp.installer``); it is a friendlier caller, not a second
implementation.
...
if sys.version_info[:2] < PYTHON_FLOOR:
    ...
    sys.stderr.write(
        f"my-localmcp's setup wizard requires Python {PYTHON_FLOOR[0]}.{PYTHON_FLOOR[1]}+ ; "
        f"found {current}.\n"
        "Install a newer interpreter and re-run:  python3.12 setup_wizard.py\n"
    )
    raise SystemExit(2)

# Make the checked-out ``my_localmcp`` package importable even before an editable
