...
def cmd_remove_client(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({
        "deprecated": True,
        "note": "`remove-client` is deprecated and will be removed in a future release; use `my-localmcp config clients remove` instead.",
    }, indent=2))
...
def cmd_reset_repo(args: argparse.Namespace) -> int:
    ...
    if not args.yes:
        print_json_text(json.dumps({"ok": False, "error": "Refusing to reset without --yes", "hint": "Run: my-localmcp reset-repo --yes"}, indent=2))
        return 2
...
def cmd_reset_all(args: argparse.Namespace) -> int:
    ...
    if not args.yes:
        print_json_text(json.dumps({"ok": False, "error": "Refusing to reset all repo context without --yes", "hint": "Run: my-localmcp reset-all --yes"}, indent=2))
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog=IDENTITY.cli_name, description="my-localmcp: deterministic repository context for Claude/Codex. Context lookup is fast/deterministic by default; Ollama ranking is opt-in.")
    sub = parser.add_subparsers(dest="command", required=True)

    p = sub.add_parser("init", help="Create the fresh my-localmcp config."); p.set_defaults(func=cmd_init)
    ...
    p = sub.add_parser("servers", help="List running my-localmcp servers registered under this app home."); p.set_defaults(func=cmd_servers)
    ...
    crp = clients_sub.add_parser("remove", help="Deregister my-localmcp from supported clients (inverse of setup).")
    ...
    p = sub.add_parser("benchmark", help="Run a repeatable my-localmcp benchmark against the current (or given) repo. Never modifies existing repository memory.")
