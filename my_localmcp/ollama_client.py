...
def start_service() -> dict[str, Any]:
    ...
    if not _is_local(base):
        return {"ok": False, "state": "unreachable", "base_url": base, "error": "remote Ollama services cannot be started by my-localmcp"}
    current = status()
...
def stop_service() -> dict[str, Any]:
    ...
    if not pid or state.get("base_url") != ollama_base_url():
        return {"ok": False, "state": "failed", "error": "refusing to stop an Ollama service not started by my-localmcp"}
    try:
