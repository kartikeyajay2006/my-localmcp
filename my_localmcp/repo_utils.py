...
def repo_root_or_cwd(repo_root: str | Path | None = None) -> Path:
    # explicit arg -> MY_LOCALMCP_REPO env -> detected git root -> configured default_root -> cwd, first one that resolves wins
    cfg = load_config()
...
def scan_repo_files(
    ...
    for dirpath, dirnames, filenames in os.walk(start):
        dirnames[:] = sorted(d for d in dirnames if not _is_excluded_dir(d, exclude_dirs) and not d.endswith(".egg-info") and d != "__pycache__" and not d.startswith(".my-localmcp"))
