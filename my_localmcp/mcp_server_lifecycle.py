...
- `my-localmcp stop` writes the stop file(s) for the targeted PIDs and waits for
...
def _servers_root() -> Path:
    # resolved dynamically from config.APP_DIR (not bound at import), so it follows MY_LOCALMCP_HOME/test APP_DIR overrides with no caller patching needed
...
def start_stop_watcher(poll: float = DEFAULT_POLL_SECONDS, on_stop: Callable[[], None] = _graceful_self_exit) -> threading.Thread:
    ...
    thread = threading.Thread(target=_loop, name="my-localmcp-stop-watcher", daemon=True)
    thread.start()
    return thread
