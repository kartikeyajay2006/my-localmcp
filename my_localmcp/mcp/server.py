...
SERVER_INSTRUCTIONS = (
    ...
    "Pass repo_root explicitly when possible. If omitted, my-localmcp uses MCP workspace roots and refuses ambiguous scope. "
...
async def _resolve_repo_root(repo_root: str, ctx: Context) -> str:
    # Fallback chain: explicit repo_root -> MY_LOCALMCP_REPO env -> config default_root -> the sole MCP file root.
    ...
    env_root = os.environ.get("MY_LOCALMCP_REPO")
    if env_root:
    ...
    if len(file_roots) == 1:
        return file_roots[0]
    if not file_roots:
        raise ValueError("No repository root was supplied and the MCP client exposed no filesystem root. Pass repo_root explicitly or set MY_LOCALMCP_REPO.")
...
def _context_prepare_worker(task: str, repo_root: str, max_files: int, token_budget: int, use_ollama: bool, model: Optional[str]) -> str:
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "my_localmcp.mcp.context_worker"], input=json.dumps(payload),
...
async def doctor(ctx: Context, repo_root: str = "auto") -> str:
    """Run the full read-only my-localmcp, repository, configuration, and Ollama health check."""
...
def main() -> None:
    ...
    # blocking mcp.run() loop, so `my-localmcp stop` (and the upgrade flow) can
