...
def _render_context_text(data: dict[str, Any]) -> str:
    ...
    lines.append("my-localmcp context")
    ...
    if repo.get("indexer_rebuild_recommended"):
        lines.append("Index note: indexer version changed; run `my-localmcp reindex` for a clean rebuild.")
    lines.append("")
...
def _sanitize_ollama_advisory(text: str, max_chars: int = 1200) -> str:
    ...
    if len(excerpt) > max_chars:
        ...
        excerpt += "\n...[ollama advisory truncated by my-localmcp]"
    return excerpt
...
def _mcp_tiny_context_text(data: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("my-localmcp context_prepare")
    ...
    lines.append("")
    lines.append("Note: Source files/tests are edit truth. my-localmcp never generates code; it only narrows context and can apply exact approved patches.")
    return "\n".join(lines)
...
def _agent_guidance(read_first: list[dict[str, Any]], interpreted: dict[str, Any]) -> list[str]:
    ...
    lines.extend([
        ...
        "If editing, produce an exact patch. my-localmcp can apply exact approved patches only.",
    ])
...
def _run_ollama_ranking(task: str, interpreted: dict[str, Any], ranked: list[dict[str, Any]], limit: int, model: str | None) -> dict[str, Any] | None:
    ...
    prompt = f"""
You are a second-pass ranking reviewer for my-localmcp. You improve the deterministic repo context; you do not replace it. Do not write source code.
