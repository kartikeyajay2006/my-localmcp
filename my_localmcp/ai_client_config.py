...
def _command_templates() -> list[tuple[str, str]]:
    ...
    base = files("my_localmcp") / "templates" / "claude-code" / "commands" / IDENTITY.slash_prefix
...
def _default_server_command() -> str:
    ...
    return str(APP_DIR / "bin" / f"my-localmcp-server{suffix}")
...
def _mcp_server_block(
    ...
    return {
        IDENTITY.mcp_server_name: {
            ...
            "args": [],
            "env": {"MY_LOCALMCP_CONFIG": _config_value(config_path)},
        }
    }
...
MY_LOCALMCP_CONFIG = "{_toml_string(_config_value(config_path))}"
# END my-localmcp
""".strip() + "\n"


def _replace_marked_block(old: str, block: str, start: str = "# BEGIN my-localmcp", end: str = "# END my-localmcp") -> str:
...
def _strip_marked_block(old: str, start: str = "# BEGIN my-localmcp", end: str = "# END my-localmcp") -> str:
...
def setup_claude_desktop(apply: bool = True) -> dict[str, Any]:
    ...
    ensure_config()
    package_path = APP_DIR / "my-localmcp.mcpb"
    return {
        ...
        "instructions": "In Claude Desktop open Settings > Extensions > Advanced settings > Install Extension, then select my-localmcp.mcpb.",
...
def remove_codex(apply: bool = True) -> dict[str, Any]:
    ...
    path = _codex_cli_config_path()
    existed = path.exists()
    block_present = existed and "# BEGIN my-localmcp" in path.read_text(encoding="utf-8")
    empty_after = False
    if apply and block_present:
        ...
        block_present_after = "# BEGIN my-localmcp" in new
...
def remove_claude_desktop(apply: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        "instructions": "In Claude Desktop open Settings > Extensions and uninstall my-localmcp. If that hangs, run `python setup.py uninstall` from the checkout first.",
