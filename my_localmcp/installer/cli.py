...
invokes exactly one operation from :mod:`my_localmcp.installer`, renders the
result, and returns an exit code. It contains NO path, migration, process,
runtime, or deletion policy -- that all lives in ``my_localmcp/installer/``.
...
version check has already passed, because ``my_localmcp`` (psutil, 3.12-only
...
def _python_floor_message(floor: tuple[int, int] = PYTHON_FLOOR) -> str:
    ...
    return (
        f"my-localmcp requires Python {floor_text}+ ; found {current}.\n"
...
# not policy -- the real ordered calls live in my_localmcp/installer/operations.py.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="setup.py",
        description=(
            "my-localmcp setup: install, reinstall, or uninstall the managed "
...
def _source_version() -> str:
    ...
    return my_localmcp.__version__
...
def _report_path_setup(
    ...
    if result.status is not OperationStatus.SUCCEEDED:
        return
    reporter.info("To use my-localmcp in a new terminal, add this to your PATH:")
    ...
    try:
        ...
        reporter.warn(f"Could not add my-localmcp to PATH: {exc}")
        return
