"""Snapshot, remove, restore, and verify my-localmcp client registrations.
...
primitives record which surfaces my-localmcp is registered on (``clients/
...
_BEGIN = "# BEGIN my-localmcp"
_END = "# END my-localmcp"
...
def _parse_claude_command(combined: str) -> str | None:
    ...
    for token in combined.replace("\n", " ").split():
        if "my-localmcp-server" in token:
            return token
    return None
