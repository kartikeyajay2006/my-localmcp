...
class ConsoleWizard:
    ...
    def _header(self, question: str = "") -> None:
        ...
        title = _ansi.cyan_bold(" my-localmcp setup wizard")
        if self.fake:
    ...
    def _menu_rows(self) -> list[tuple[str, str, str]]:
        ...
        if not self.detected.is_installed:
            return [
                (OP_INSTALL, "Install my-localmcp",
    ...
    def _main_menu(self) -> str:
        ...
        while True:
            ...
            self._explain(
                "my-localmcp is a local MCP server that gives your AI tools fast,",
    ...
    def _phase_clients(self) -> None:
        ...
        prompt = ("Which clients should stay connected?"
                  if defaults_to_registered else "Which clients should connect to my-localmcp?")
        self._header(prompt)
        self._explain(
            "my-localmcp registers itself with each AI client you pick so that",
    ...
    def _phase_ollama_baseurl(self) -> None:
        ...
        self._explain(
            "This is the base URL my-localmcp will talk to Ollama on -- almost",
    ...
    def _phase_path(self) -> None:
        ...
        self._header("Add my-localmcp to PATH")
        self._explain(
            "Add the managed my-localmcp command to your user PATH so new terminals",
            "can run it directly. This can be removed later from your shell profile or user PATH.",
        )
        self.state.add_to_path = self._ask_yesno(
            "Add my-localmcp to PATH?", default=True,
        )
    ...
    def _phase_uninstall_scope(self) -> None:
        self._header("Uninstall my-localmcp")
        self._explain(
    ...
    def _confirm(self, *, default_proceed: bool = True) -> None:
        ...
        if missing:
            for line in _wrap(
                "The following selected model(s) are not pulled yet -- my-localmcp "
