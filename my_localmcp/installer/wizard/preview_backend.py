...
exist yet, the starting state is seeded from MY_LOCALMCP_WIZARD_PREVIEW_STATE:
...
# behind, instead of always restarting from the same MY_LOCALMCP_WIZARD_PREVIEW_STATE
...
def _seed_state() -> dict[str, Any]:
    # MY_LOCALMCP_WIZARD_PREVIEW_STATE "healthy"/"installed"/"returning" -> seeds an already-installed simulated state; default "absent" -> fresh clone
...
def _fake_client_meta(key: str) -> tuple[str, str, bool]:
    ...
    if key == "claude-code":
        return (_join(".claude", "commands", "my-localmcp"),
    ...
    return (_join(".my-localmcp", "my-localmcp.mcpb"),
...
class PreviewBackend:
    ...
    def _simulate_install_like(self, state: WizardState, emit: EmitFn) -> OperationOutcome:
        ...
        return OperationOutcome(
            ...
            next_command="my-localmcp doctor",
        )
