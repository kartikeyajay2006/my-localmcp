...
``my_localmcp.installer`` operations, ``my_localmcp.config``,
``my_localmcp.ollama_client``, and ``my_localmcp.ai_client_config``. This module
...
class LiveBackend:
    ...
    def __init__(self) -> None:
        ...
        self._source_root = Path(my_localmcp.__file__).resolve().parent.parent
        self._source_version = my_localmcp.__version__
    ...
    def client_options(self) -> list[ClientOption]:
        ...
        mcpb = str(self._paths.root / "my-localmcp.mcpb")
        rows = {
            "claude-code": (
                claude_code,
            ...
            "codex": (
                codex,
                "A marked my-localmcp block is written into config.toml "
    ...
    def ollama_info(self) -> OllamaInfo:
        ...
        if reachable and installed:
            ...
            detail = (f"Ollama not reachable at {base_url} "
                      f"({probe.get('error') or state}). my-localmcp works without it.")
        return OllamaInfo(
        ...
        if (result.status is OperationStatus.SUCCEEDED
            ...
            if state.add_to_path:
                try:
                    ...
                    emit(StepEvent("warning", f"Could not add my-localmcp to PATH: {exc}"))
    ...
    def _outcome_from_result(
        ...
        if ok:
            ...
            return OperationOutcome(
                ...
                next_command=None if op == OP_UNINSTALL else "my-localmcp doctor",
            )
    ...
    def apply_ollama_config(self, state: WizardState, emit: EmitFn) -> OperationOutcome:
        ...
        return OperationOutcome(
            ...
            next_command="my-localmcp ollama status",
        )
