...
class ManagedPaths:
    ...
    def from_environment(
        ...
        *,
        ...
        # MY_LOCALMCP_HOME env override wins, else default to ~/.my-localmcp; platform auto-detected unless given
        environment = os.environ if environ is None else environ
        resolved_home = Path.home() if home is None else Path(home)
        configured = environment.get("MY_LOCALMCP_HOME")
        root = Path(configured).expanduser() if configured else resolved_home / ".my-localmcp"
    ...
    def cli_executable(self) -> Path:
        return self.executable_dir / f"my-localmcp{self.executable_suffix}"

    @property
    def server_executable(self) -> Path:
        return self.executable_dir / f"my-localmcp-server{self.executable_suffix}"
    ...
    def validate_destructive_root(self) -> Path:
        # guards before any rm-style operation: root must resolve, not be a filesystem root, not be $HOME, and (unless allow_test_root) must be literally named .my-localmcp
        try:
        ...
        if not self.allow_test_root and resolved.name != ".my-localmcp":
            raise UnsafeManagedRoot(
                f"Refusing unexpected managed root (expected .my-localmcp): {resolved}"
            )
        return resolved
