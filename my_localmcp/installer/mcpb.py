...
Node/``npx``). The staged layout is:
    ...
    my_localmcp/**    <- repo root, honoring packages/claude-desktop/mcpb/.mcpbignore
...
def _next_free_path(package_dir: Path, version: str) -> Path:
    ...
    base = package_dir / f"my-localmcp-v{version}.mcpb"
    if not base.exists():
        return base
    counter = 2
    while True:
        candidate = package_dir / f"my-localmcp-v{version}-{counter}.mcpb"
...
def build_mcpb(source_root: Path | str, version: str) -> Path | None:
    # staging/manifest.json + repo root files + my_localmcp/** (minus exclusions) -> zipped into the versioned bundle
