...
_PACKAGE_VERSION_SNIPPET = "import my_localmcp; print(my_localmcp.__version__)"
_SERVER_IMPORT_SNIPPET = "import my_localmcp.mcp.server"
_DOCTOR_SNIPPET = "import sys; from my_localmcp.mcp.system import doctor; sys.stdout.write(doctor())"
_CANONICAL_PATHS_SNIPPET = (
    "import json; from my_localmcp import config;"
...
class EnvCommandRunner(Protocol):
    ...
    environment (``MY_LOCALMCP_HOME``) rather than the caller's own."""
...
async def _probe_mcp_handshake_async(paths: ManagedPaths, *, timeout: float) -> tuple[bool, str]:
    ...
    # isolated MY_LOCALMCP_HOME so the probe never touches the real managed registry; finally-block cleanup so a failed probe never leaves a stray registered PID
    ...
    env = {**os.environ, "MY_LOCALMCP_HOME": str(paths.root)}
    params = StdioServerParameters(
        command=str(python_executable),
        args=["-m", "my_localmcp.mcp.server"],
        env=env,
    )
...
def _check_cli_startup(paths: ManagedPaths, *, runner: CommandRunner) -> VerificationCheck:
    # `my-localmcp --help` actually runs to completion
    result = runner.run(
...
def _check_server_import(location_python: Path, *, runner: CommandRunner) -> VerificationCheck:
    # `import my_localmcp.mcp.server` succeeds under the managed interpreter
    result = runner.run(
...
def _check_canonical_config_path(
    ...
    # config.config_path() under managed env resolves to exactly paths.config/config.yaml -- catches MY_LOCALMCP_HOME drift
    ...
    if payload is None:
        ...
        return VerificationCheck(
            ...
            ok=False,
            details=details,
            recovery="Ensure MY_LOCALMCP_HOME resolves consistently for the managed runtime.",
        )
    ...
    return VerificationCheck(
        ...
        ok=ok,
        details=details,
        recovery="" if ok else "MY_LOCALMCP_HOME / config layout is out of sync with the managed paths.",
    )
...
def _check_canonical_database_path(
    ...
    return VerificationCheck(
        ...
        ok=ok,
        details=details,
        recovery="" if ok else "MY_LOCALMCP_HOME / sqlite layout is out of sync with the managed paths.",
    )
...
def _check_doctor(paths: ManagedPaths, *, env_runner: EnvCommandRunner) -> VerificationCheck:
    # runs `my-localmcp doctor` under the managed env and inspects its output
    env = {**os.environ, "MY_LOCALMCP_HOME": str(paths.root)}
    result = env_runner.run(
    ...
    if not result.ok:
        ...
        return VerificationCheck(
            ...
            ok=False,
            details=details,
            recovery="Run `my-localmcp doctor` directly to inspect the failure.",
        )
    try:
        payload = json.loads(result.stdout)
    except (ValueError, json.JSONDecodeError):
    ...
    return VerificationCheck(
        ...
        ok=ok,
        details=details,
        recovery="" if ok else "Run `my-localmcp doctor` directly to inspect the failing checks.",
    )
...
def _check_claude_desktop_manual(paths: ManagedPaths) -> VerificationCheck:
    ...
    return VerificationCheck(
        ...
        recovery=(
            ...
            "Extension, then select my-localmcp.mcpb."
        ),
    )
