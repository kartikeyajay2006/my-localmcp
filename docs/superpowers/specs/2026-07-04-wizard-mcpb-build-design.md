...
`packages/claude-desktop/my-localmcp-v<version>.mcpb` to a fixed-name local copy
at `~/.my-localmcp/my-localmcp.mcpb`, but the stdlib Python installer/wizard that
...
`~/.my-localmcp/` copy (deemed useless). Instead, the wizard should **build** the
...
3. **No-overwrite numeric counter.** Base name `my-localmcp-v{version}.mcpb`; if it
   exists, try `my-localmcp-v{version}-2.mcpb`, `-3`, … until a free name is found.
4. **Out of scope:** the `~/.my-localmcp/my-localmcp.mcpb` copy (left untouched),
...
Verified against the committed `packages/claude-desktop/my-localmcp-v1.1.0.mcpb`
...
- `my_localmcp/**` — excluding `tests/`, `__pycache__/`, `*.pyc`, `.venv/`
...
### Component 1 — `my_localmcp/mcpb_build.py` (new module)

```python
def build_mcpb(source_root: Path, version: str) -> Path | None:
    """Pack packages/claude-desktop/my-localmcp-v{version}.mcpb from a source
    checkout.
...
1. Resolve `pkg_dir = source_root / "packages" / "claude-desktop"` and
   ...
   `None` (dev-only gate).
2. Compute the target path: `pkg_dir / f"my-localmcp-v{version}.mcpb"`; if it
...
### Component 2 — wizard hook (`my_localmcp/wizard/real_backend.py`)
...
(`my-localmcp-v1.0.10.mcpb`, `my-localmcp-v1.1.0.mcpb`) and
...
`packages/claude-desktop/mcpb/` + `my_localmcp/` + `README.md` + `pyproject.toml`):

- Produces `my-localmcp-v{version}.mcpb`.
- A second call produces `my-localmcp-v{version}-2.mcpb` and leaves the first
  intact (no overwrite).
- Bundle contains `manifest.json`, `server.py`, `README.md`, `pyproject.toml`, and
  `my_localmcp/` files; excludes `tests/`, `__pycache__/`, `*.pyc`.
...
- `python -m compileall -q my_localmcp` clean.
- Manually diff the Python-built bundle's file list against the committed
  `my-localmcp-v1.1.0.mcpb` to confirm content parity.
