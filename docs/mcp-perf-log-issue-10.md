# my-localmcp MCP Performance / Benchmarking Log — Issue #10

Every my-localmcp CLI command exercised during the issue-10 code-quality audit,
...
Environment: Windows 11, host Python 3.14, `my-localmcp` installed under
`~/.my-localmcp`. Ollama fast model `qwen3:8b`, summary model `qwen3-coder:30b`
...
- **Invocation:** `my-localmcp context "audit tools.py context_prepare ranking and excerpt centering complexity: context_prepare, _add_candidate, _best_heading_section" --repo-root . --token-budget 1000 --format text`
- **Wall clock:** 2.508 s (`user`+`sys` ≈ 0.03 s — nearly all time is the child
  `my-localmcp-server`/index work, not the shell).
...
- **Invocation:** `my-localmcp ollama status`
...
- **Invocation:** `my-localmcp context "review context_prepare for KISS/SOLID complexity: context_prepare" --repo-root . --token-budget 800 --ollama-rank --format json`
...
- **Notable win:** with `--ollama-rank`, the reranker named the canonical
  `my_localmcp/tools.py` as #1, effectively correcting the deterministic
...
- **Invocation:** `my-localmcp lookup "index_repo" --repo-root .`
...
- **Result:** 4 symbol hits + 2 file hits, each with file_path + start/end line.
  ...
  canonical `my_localmcp/repo_memory.py` symbol first).
...
- **Invocation:** `my-localmcp file my_localmcp/query.py --repo-root . --around-line 61 --context-lines 20`
...
- **Invocation:** `my-localmcp test-determinism "debug repository indexing: index_repo, refresh" --repo-root . --runs 5`
...
- **Invocation (cold):** `my-localmcp summarize docs/ARCHITECTURE.md --repo-root . --heading "Safety model"`
