...
def isolated_app_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    Setting MY_LOCALMCP_HOME alone is NOT enough for in-process isolation:
    config.py's APP_DIR/CONFIG_PATH are computed once at module import time
    from that env var and never re-derived live the way MY_LOCALMCP_CONFIG
    ...
    subprocess) silently falls through to the real ~/.my-localmcp. Mirror
    isolated_config's proven pattern and patch the module globals directly too.
    """

    app_home = tmp_path / ".my-localmcp"
    monkeypatch.setenv("MY_LOCALMCP_HOME", str(app_home))
